...
import { Loader2, Lock, PenLine, ArrowRight } from 'lucide-react'
    ...
    <NativeToolLayout title="Signature" description="Sign any PDF by dragging your signature image." actions={pdfData && !pdfData.isLocked && !downloadUrl && <ActionButton />}>
      ...
      {!pdfData ? (
        ...
        >
          ...
          <p className="text-sm text-gray-400">Tap to load your document for signing</p>
        </button>
      ) : pdfData.isLocked ? (
        <div className="max-w-md mx-auto p-8 bg-white dark:bg-zinc-900 rounded-3xl text-center border border-gray-100 dark:border-white/5">
          <Lock size={32} className="mx-auto mb-4 text-rose-500" />
          <h3 className="text-xl font-bold dark:text-white mb-1">Protected File</h3>
          <p className="text-sm text-gray-400 mb-6">Enter the password to unlock and sign this PDF</p>
          <input type="password" value={unlockPassword} onChange={(e) => setUnlockPassword(e.target.value)} placeholder="Enter Password" className="w-full bg-gray-50 dark:bg-black rounded-2xl px-6 py-4 border border-transparent focus:border-rose-500 outline-none font-bold text-center mb-4 dark:text-white" autoFocus />
          <button onClick={handleUnlock} disabled={isProcessing || !unlockPassword} className="w-full bg-rose-500 text-white p-4 rounded-2xl font-black uppercase tracking-widest text-xs disabled:opacity-50">{isProcessing ? '...' : 'Unlock'}</button>
        </div>
      ) : (
            ...
            <>
              ...
              <div className="flex gap-4">
                <button onClick={() => signatureInputRef.current?.click()} className="flex-1 p-4 bg-gray-50 dark:bg-zinc-800 text-gray-900 dark:text-white border border-gray-100 dark:border-white/5 rounded-2xl font-black uppercase text-xs hover:border-rose-500 transition-all">
                  <span className="flex items-center justify-center gap-2"><PenLine size={16}/> Upload Signature</span>
                </button>
              </div>
